...
void * _memcpy(void * dst, const void * src, u8 cnt){
...
void NRF24L01_RX_Mode(void)
{
  ...
  NRF24L01_Write_Reg(WRITE_REG_NRF + RF_CH, ch);        //   设置信道工作为2.4GHZ，收发必须一致
  NRF24L01_Write_Reg(WRITE_REG_NRF + RX_PW_P0, st_ps); //设置接收数据长度，本次设置为32字节
  ...
  NRF24L01_Write_Reg(WRITE_REG_NRF + CONFIG, 0x0b);    //power up  1: PRX  配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
...
void NRF24L01_RT_Mode(void)
{
  ...
  NRF24L01_Write_Buf(WRITE_REG_NRF+TX_ADDR,(u8*)tx_addr,addr_width);//写TX节点地址 	
  NRF24L01_Write_Buf(WRITE_REG_NRF+RX_ADDR_P0,(u8*)rx_addr,addr_width);//写RX节点地址
  NRF24L01_Write_Reg(WRITE_REG_NRF+EN_AA,st_aa);     //使能通道0的自动应答    
  NRF24L01_Write_Reg(WRITE_REG_NRF+EN_RXADDR,0x01); //使能通道0的接收地址  
  NRF24L01_Write_Reg(WRITE_REG_NRF+RF_CH,ch);       //设置RF通道为0,通讯频率2400+ RH_CH mhz 双方一致
  NRF24L01_Write_Reg(WRITE_REG_NRF+RX_PW_P0, st_ps);//选择通道0的有效数据宽度
  // NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,0); 
  NRF24L01_Write_Reg(WRITE_REG_NRF+RF_SETUP,txReg);  //设置TX发射参数,0db增益,2Mbps,低噪声增益开启   
...
void nrf_RxTx(u8 mod_nrf,u8 *buf)
{
  ...
  else if(mod_nrf == 'r')//接收模式
  {
    ...
    {
      GPIO_WriteLow(NRF24L01_CE_PORT,NRF24L01_CE_PIN);
      NRF24L01_Read_Buf(RD_RX_PLOAD,buf, st_ps);//读取数据 存入数组
...
u8 NRF_Get_State(void)
{
  ...
  if(status & RX_OK)
  {
    NRF24L01_Read_Buf(RD_RX_PLOAD,rec_buf, st_ps);//读取数据
    NRF24L01_Write_Reg(FLUSH_RX, NOP);    // 清除RX FIFO寄存器 
    //printf("%s",rec_buf);
